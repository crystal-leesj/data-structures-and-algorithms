...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    result.push(ingredient.slice(ingredient.indexOf(' ', 3)).substring(1));
  })
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters >= str.length) {
    ...
    splitByChar.splice(str.length-numberOfCharacters, numberOfCharacters);
    let result = splitByChar.join('');
    return result;
  }
};
